...
class List extends Component {
    ...
    componentDidMount() {
        const category = this.props.scene.route.category
        if (category == 1) {
            this.props.refresh('独家购')
        } else if (category == 2) {
            this.props.refresh('领劵购')
        }
    }
    ...
    render() {
        ...
        const category = this.props.scene.route.category
        const text = category == 0 ? "确认收货后即可获得返现(请勿提现确认收货)" :
            '务必先领卷再下单，否则无法获得活动活动折扣价'
        const tipView = ()=> {
            return (
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        refresh: (title: string)=> {
            dispatch(navigateRefresh({title}))
        },
