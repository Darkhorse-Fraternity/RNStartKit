...
function send({
    ...
    let request = method == 'GET' ? new Request(addParams(urlpath, params), {
        method: method,
        headers: httpHeader
    }) : new Request(urlpath, {method: method, headers: httpHeader, body: JSON.stringify(params)});
...
module.exports = {
    ...
    request({
        ...
        let request = method == 'GET' ? new Request(addParams(urlpath, params), {method: method, headers: httpHeader})
            : new Request(urlpath, {method: method, headers: httpHeader, body: JSON.stringify(params)});
        let requestPromise = Promise.race([
            fetch(request, {credentials: 'include'}),
            new Promise(function (resolve, reject) {
